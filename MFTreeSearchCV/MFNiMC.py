...
class MFNiMC(object):
    ...
    def __init__(self, NiMC, batch_size, rand_sampling, eval_mult):
        self.NiMC = NiMC
        self.Theta_index_non_const_dims = np.where(NiMC.Theta[1][:, 1] - NiMC.Theta[1][:, 0] > 0)[0]
        ...
        self.domain_bounds = NiMC.Theta[1][self.Theta_index_non_const_dims, :]
        self.domain_dim = len(self.Theta_index_non_const_dims)
        self.discrete_set = NiMC.Theta[0]
        self.num_trees = len(NiMC.Theta[0])
    ...
    def get_full_state(self, state):
        ...
        state_ = self.NiMC.Theta[1][:, 0].copy()
